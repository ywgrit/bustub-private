...
class RowMatrix : public Matrix<T> {
 public:
  ...
  void FillFrom(const std::vector<T> &source) override {
    int len = source.size();
    if (len != Matrix<T>::rows_ * Matrix<T>::cols_) {
...
class RowMatrixOperations {
 public:
  ...
  static std::unique_ptr<RowMatrix<T>> Multiply(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB) {
    ...
    auto result = std::unique_ptr<RowMatrix<T>>(new RowMatrix<T>(rowa, colb));
  ...
  static std::unique_ptr<RowMatrix<T>> GEMM(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB,
    ...
    // TODO(P0): Add implementation
    /* int rowa = matrixA->GetRowCount(), cola = matrixA->GetColumnCount(), rowb = matrixB->GetRowCount(), colb =
     * matrixB->GetColumnCount(), rowc = matrixC->GetRowCount(), colc = matrixC->GetColumnCount(); */
