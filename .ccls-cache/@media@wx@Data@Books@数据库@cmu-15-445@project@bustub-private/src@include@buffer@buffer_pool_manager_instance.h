...
class BufferPoolManagerInstance : public BufferPoolManager {
 public:
  ...
  /** @return page_table_ in the buffer pool */
  std::unordered_map<page_id_t, frame_id_t> GetPageTable() { return page_table_; }
 ...
 protected:
  ...
  /** Number of pages in the buffer pool. i.e capacity */
  ...
  Replacer *replacer_;
  /** List of free pages. frame_id is the real id of page in bufferpool, not the page_id, page_id is just the logical id
   * of page, for example, page_id maybe be 5000 but there just 4000 pages in bufferpool */
