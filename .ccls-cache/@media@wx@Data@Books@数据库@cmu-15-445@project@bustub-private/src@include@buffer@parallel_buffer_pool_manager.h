...
class ParallelBufferPoolManager : public BufferPoolManager {
 ...
 protected:
  ...
  /** BufferPoolManagerInstances which parallel_buffer_pool_manager manage */
  std::vector<BufferPoolManagerInstance *> instances_;
  /** Number of pages in the buffer pool of single BufferPoolManagerInstance. i.e capacity */
  const uint32_t num_instances_;
  /** next insert location */
  const size_t pool_size_;
  /** Number of instances */
  size_t next_instance_;
  /** This latch protects next_instance_. */
  std::mutex latch_;
};
}  // namespace bustub
