...
class ExtendibleHashTable {
 public:
  ...
  /**
   * Print the directory page.
   */
  void PrintMemberDirectory();
  ...
  /**
   * Convert the raw page to bucket page
   *
   * @param page be converted to bucket page
   * @return a pointer to the bucket page
   */
  HASH_TABLE_BUCKET_TYPE *GetBucketPageData(Page *page);

  /**
   * Fetch the bucket indexed by bucket_idx
   * @return pointer to the bucket indexed by bucket_idx
   */
  HASH_TABLE_BUCKET_TYPE *GetBucket(uint32_t bucket_idx);
 ...
 private:
  ...
  // latch for protect create directory operation
  ...
  // Readers includes inserts and removes, writers are splits and merges, so only directory is going to be modified can
  // we use writers.
