...
void HashTableDirectoryPage::IncrGlobalDepth() {
  ...
  uint32_t size = Size();
  for (uint32_t ori_index = 0, new_index = size; ori_index < size; ori_index++, new_index++) {
    bucket_page_ids_[new_index] = bucket_page_ids_[ori_index];
    local_depths_[new_index] = local_depths_[ori_index];
  }
  global_depth_++;
}

uint32_t HashTableDirectoryPage::GetSplitImageIndex(uint32_t bucket_idx) {  // split image is the brother image
...
bool HashTableDirectoryPage::CanShrink() {  // only all local_depth less than global_depth, can shrink global_depth
  uint32_t size = Size();
